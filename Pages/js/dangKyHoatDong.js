// File: js/dangKyHoatDong.js

document.addEventListener('DOMContentLoaded', function() {
    // --- CẤU HÌNH ---
    const API_URL = 'http://localhost:5114/api/HoatDongs'; // Cổng 5114 của bạn
    ...
    // --- [THÊM MỚI] Biến lưu thông tin sinh viên ---
    const CURRENT_STUDENT_ID = 1; // Giả định ID sinh viên đang đăng nhập
    let studentInfo = { HoTen: "...", MSSV: "..." };

    // --- [THÊM MỚI] Hàm tải thông tin sinh viên ---
    async function loadStudentInfo() {
        try {
            // Lưu ý: Cổng API phải khớp (7006 hay 5114 tùy máy cậu)
            const response = await fetch(`https://localhost:7006/api/SinhViens/${CURRENT_STUDENT_ID}`);
            const data = await response.json();
            if (data.ThongTin) studentInfo = data.ThongTin;
        } catch (e) { console.error("Lỗi tải SV:", e); }
    }

    // Gọi hàm này ngay khi trang tải
    loadStudentInfo();
    ...
    async function loadActivities() {
        ...
        try {
            // Gọi API kèm theo Tab và Search
    ...
    // --- 2. HÀM VẼ GIAO DIỆN (PHIÊN BẢN SỬA LỖI CHỮ HOA/THƯỜNG) ---
        ...
        if (!activities || activities.length === 0) {
            container.innerHTML = `
                <div class="col-span-full text-center py-16 bg-white rounded-xl border border-dashed border-gray-300">
                    ...
                    <p class="text-gray-500">Không tìm thấy hoạt động nào ở mục này.</p>
                </div>`;
            return;
        }

        activities.forEach(act => {
            // --- 1. CHUẨN HÓA DỮ LIỆU (Bắt cả chữ Hoa và chữ Thường) ---
            // Backend có thể trả về 'TenHoatDong' hoặc 'tenHoatDong', ta lấy cái nào có dữ liệu
            const id = act.Id || act.id;
            const ten = act.TenHoatDong || act.tenHoatDong;
            const poster = act.PosterUrl || act.posterUrl || 'images/banner1.jpg'; // Ảnh mặc định nếu thiếu
            ...
            // Lấy trạng thái từ Backend gửi xuống (An toàn nhất)
            ...
            // Kiểm tra: Nếu backend bảo "KetThuc" thì là kết thúc
            ...
            // --- 3. VẼ GIAO DIỆN ---
            // Lưu ý: Chỗ onclick phải truyền đúng 'id' và 'ten' đã xử lý ở trên
            const cardHtml = `
                <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-shadow border border-gray-100 flex flex-col h-full">
                    <div class="relative h-48 overflow-hidden group">
                        ...
                        <div class="absolute top-2 right-2 bg-yellow-400 text-blue-900 text-xs font-bold px-2 py-1 rounded shadow">
                            +${diem} ĐIỂM
                        </div>
                    ...
                    <div class="p-5 flex-grow">
                        <h3 class="text-lg font-bold text-gray-800 mb-2 line-clamp-2 hover:text-blue-700 cursor-pointer" title="${ten}">${ten}</h3>
    ...
    // --- 5. GỬI ĐĂNG KÝ (ĐÃ SỬA LỖI) ---
    btnConfirm.addEventListener('click', async () => {
        const actId = document.getElementById('modalActivityId').value;
        const mssv = document.getElementById('modalMssv').value;
        // Lấy thẻ input file (Việt kiểm tra lại id bên file HTML xem có đúng là 'fileMinhChung' không nhé)
        ...
        try {
            // 1. Tạo đối tượng FormData (Thay vì JSON)
            const formData = new FormData();
            formData.append('hoatDongId', actId);
            formData.append('mssv', mssv);
            
            // 2. Kiểm tra nếu có file ảnh thì nhét vào
            if (fileInput && fileInput.files[0]) {
                formData.append('minhChung', fileInput.files[0]);
            }

            // 3. Gửi bằng fetch (LƯU Ý: Không được để Content-Type là application/json)
            const response = await fetch(`${API_URL}/DangKy`, {
                method: 'POST',
                // KHÔNG thêm headers 'Content-Type': 'application/json' ở đây
                // Để trình duyệt tự động nhận diện là Form
                body: formData 
            });

            // 4. Xử lý kết quả
            // Vì backend trả về Ok(new { message... }) nên vẫn đọc json bình thường
            const result = await response.json();

            if (response.ok) {
                alert("🎉 " + result.message);
                hideModal();
                loadActivities(); // Tải lại để cập nhật số slot
            } else {
                // Nếu lỗi, hiển thị chi tiết
                console.log(result); // Soi log nếu cần
                alert("⚠️ Lỗi: " + (result.title || "Vui lòng kiểm tra lại thông tin"));
            }

        } catch (error) {
            console.error(error);
            alert("Lỗi kết nối server hoặc code JS bị sai.");
        } finally {
